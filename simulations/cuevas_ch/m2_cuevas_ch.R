...
for (j in 1:6) {
  for (n in 1:1000) {
    ...
    x <- cbind(t^1*(1-t)^(6-1) + rnorm(25, mean=0, sd=sigma[j]),
    ...
    y <- cbind(t^2*(1-t)^(6-2) + rnorm(25, mean=0, sd=sigma[j]),
    ...
    fanova1 <- fanova.tests(data, group.label = label, test = "CH")
    t_val_star[n,j] <- as.numeric(as.character(unlist(fanova1$CH[2])))
  }
}
...
for (j in 1:6) {
  for (n in 1:1000) {
    ...
    x <- cbind(t^1*(1-t)^(6-1)+BM(x=0, t0=0, T=1, N=24)*sigma[j],
    ...
    y <- cbind(t^2*(1-t)^(6-2)+BM(x=0, t0=0, T=1, N=24)*sigma[j],
